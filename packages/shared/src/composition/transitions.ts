...
interface VisualParts { opacity: number, tx: number, ty: number, scale: number, clipPath?: string }

function visualParts(type: TransitionType, direction: TransitionDirection | undefined, role: 'in' | 'out', p: number): VisualParts {
  if (type === 'slide') {
    // exit 单位向量 = 旧内容离场方向;入场从反侧进入。名称方向 = 运动方向(与 wipe 对齐)。
    const exit = direction === 'left' ? [-1, 0] : direction === 'right' ? [1, 0] : direction === 'up' ? [0, -1] : [0, 1]
    const [ex, ey] = exit
    if (role === 'out')
      return { opacity: 1, tx: ex * 100 * p, ty: ey * 100 * p, scale: 1 }
    return { opacity: 1, tx: -ex * 100 * (1 - p), ty: -ey * 100 * (1 - p), scale: 1 }
  }
  if (type === 'wipe') {
    if (role === 'out')
      return { opacity: 1, tx: 0, ty: 0, scale: 1 } // 出场不裁,入场盖住(z 序入场在上)
...
function format({ opacity, tx, ty, scale, clipPath }: VisualParts): TransitionRenderProps {
  return {
    opacity,
 ...
 */
export function transitionVisual(type: TransitionType, direction: TransitionDirection | undefined, role: 'in' | 'out', p: number): TransitionRenderProps {
  return format(visualParts(type, direction, role, p))
}

/** item 在某帧因转场获得的渲染 props。absFrame = 合成绝对帧(调用方传 item.from + useCurrentFrame())。 */
export function getTransitionRenderProps(state: UndoableState, item: EditorStarterItem, absFrame: number): TransitionRenderProps {
  ...
  // 旧存档没有 transitions 字段(deserializeState 会回填 {});空表自然遍历 0 次
  for (const t of dictValues(state.transitions ?? {})) {
    ...
    ty += v.ty
    scale *= v.scale
    if ((v.clipPath != null && v.clipPath !== ''))
      clipPath = v.clipPath // wipe 出场无 clipPath → 每 item 至多一个
  }
