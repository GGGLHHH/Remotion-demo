...
import type { AudioItem, Crop, EditorStarterAsset, GifItem, ImageItem, VideoItem } from '../../types'
import { Gif } from '@remotion/gif'
import { Audio, Video } from '@remotion/media'
import { Img } from 'remotion'

/** crop 渲染：外层裁剪窗口，内层原图按比例放大平移 */
function croppedStyle(crop: Crop | null, asset: { width: number, height: number }, itemW: number, itemH: number): React.CSSProperties {
  if (!crop)
 ...
 * 音频条目无视觉，沿用基础淡变对。
 */
export function volumeWithFades(item: VideoItem | AudioItem): number | ((f: number) => number) {
  const fi
...
export const VideoItemRenderer: React.FC<{
  ...
  if (!(url != null && url !== '') || !asset || asset.type !== 'video')
    return null
  return (
...
export const ImageItemRenderer: React.FC<{
  ...
  if (!(url != null && url !== '') || !asset || asset.type !== 'image')
    return null
  return (
    <div style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'relative' }}>
