import type { UndoableState, VideoAsset, VideoItem } from '@gedatou/shared'
import { createCaptionAsset, createCaptionsItem, createEmptyState, createSolidItem, createTextItem, createTrack, dictValues, newId } from '@gedatou/shared'
...
function addVideo(state: UndoableState, trackId: string, opts?: Partial<VideoItem>) {
...
describe('trimItem', () => {
  it('start：from/dur/trimBefore 联动', () => {
    const { state, t1 } = build()
  ...
  it('start：trimBefore 钳到 0（不能露出素材开头之前）', () => {
    const { state, t1 } = build()
    const { item } = addVideo(state, t1.id, { from: 30, trimBefore: 5 })
    const next = trimItem(state, item.id, 'start', -10)
    const it = next.items[item.id] as VideoItem
...
describe('rollEdit', () => {
  ...
  it('a 为媒体：出点不能超素材末尾；delta 0 时引用不变', () => {
    const { state, t1 } = build()
    // 素材 300 帧全占 ⇒ A 无法再扩展
  ...
  it('b 为媒体：trimBefore=0 时入点无法左移；右移增加 trimBefore', () => {
    const { state, t1 } = build()
...
describe('splitItemsAtFrame', () => {
  ...
  it('视频分割：左半清淡出+音频淡出，右半清淡入+音频淡入，另一侧保留', () => {
    const { state, t1 } = build()
    ...
    const next = splitItemsAtFrame(state, 30, [item.id])
    const parts = dictValues(next.items) as VideoItem[]
    const left = parts.find(p => p.from === 10)!
    const right = parts.find(p => p.from === 30)!
...
describe('tracks', () => {
  ...
  it('removeEmptyTracks 保留有内容的和至少一条', () => {
...
describe('maxExtendFrames', () => {
  it('左 = min(trimBefore 换算, from)；右 = 素材剩余', () => {
    const { state, t1 } = build()
...
describe('detachTransitionsOf', () => {
  ...
  it('两侧都参与(A-B 与 B-C 的 B)⇒ 两个都断开', () => {
    expect(detachTransitionsOf(table, 'B')).toEqual({})
  })
  it('与转场无关的块 ⇒ 返回原引用(不触发无谓写)', () => {
