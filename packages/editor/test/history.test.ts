...
describe('undo/redo', () => {
  it('commit 更新推入 past、清空 future；undo 回滚、redo 重做', () => {
  ...
  it('updater 返回原引用 ⇒ 不入栈不变更', () => {
    store().updateUndoable(s => s)
    expect(store().past).toHaveLength(0)
  })

  it('连续 commit:false + commitPending ⇒ 一条撤销记录', () => {
    const before = store().undoable
  ...
  it('commitPending 无 pending 时为 no-op', () => {
    store().commitPending()
    expect(store().past).toHaveLength(0)
  })
  ...
  it('deleteSelected 移除 items 并清空选中，一条撤销记录', () => {
