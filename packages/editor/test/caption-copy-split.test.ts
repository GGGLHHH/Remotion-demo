...
describe('复制字幕块', () => {
  ...
  it('副本拿到独立的字幕数据 ⇒ 改副本不影响原块', () => {
    api.getState().setSelected([built.capId])
    duplicateSelection(api)
    const copy = api.getState().undoable.items[copyIdOf()]
    if (copy!.type !== 'captions')
      throw new Error('not captions')
...
describe('切分字幕块', () => {
  it('两半各拿一份数据，右半内容按切点平移', () => {
    const { s, capId } = build()
    const next = splitItemsAtFrame(s, 30, [capId]) // 30 帧 = 1 秒
    const halves = dictValues(next.items).filter(i => i.type === 'captions')
  ...
  it('切源块 ⇒ 绑定的字幕在同一处一起切，右半字幕改绑右半源', () => {
    const { s, srcId, capId } = build()
    const next = splitItemsAtFrame(s, 30, [srcId]) // 只选中源块
    const caps = dictValues(next.items).filter(i => i.type === 'captions')
