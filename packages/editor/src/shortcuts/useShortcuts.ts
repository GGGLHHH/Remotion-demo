  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent): void => {
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        e.preventDefault()
        if ((store.selectedTransitionId != null && store.selectedTransitionId !== '')) {
      ...
      if (e.key === 'Escape') {
        if ((store.itemSelectedForCrop != null && store.itemSelectedForCrop !== '')) {
          store.setItemSelectedForCrop(null)
        }
        else if ((store.textItemEditing != null && store.textItemEditing !== '')) {
