...
import { dictEntries, dictValues, expandSelectionWithGroups, groupFromSelection, newId, pruneGroups, reorderGroup, ungroupBySelection } from '@gedatou/shared'

// 选择 slice:选中项/转场 + 成组/拆组/重排 + 删除。写 undoable 一律经 get().updateUndoable(history slice)。
export function createSelectionSlice(set: EditorSet, get: EditorGet): Pick<
  EditorStore,
  | 'selectedItemIds'
    ...
    deleteSelected: () => {
      ...
      updateUndoable((s) => {
        ...
        for (const o of dictValues(items)) {
          if (o.type === 'captions' && (o.sourceItemId != null && o.sourceItemId !== '') && gone.has(o.sourceItemId))
            delete items[o.id]
        }
        ...
        const transitions = Object.fromEntries(
          dictEntries(s.transitions ?? {}).filter(
