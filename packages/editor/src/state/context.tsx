...
import type { EditorInstanceRefs } from './instance-refs'
import type { EditorDeps } from './runtime'
import type { EditorInitialState, EditorStore, EditorStoreApi } from './store'
import { createContext, use, useRef } from 'react'
import { useStore } from 'zustand'
import { createInstanceRefs } from './instance-refs'
import {
  createEditorStore,
...
export function EditorProvider({
  ...
  if (!storeRef.current)
    storeRef.current = store ?? createEditorStore(initialState)
  const refsRef = useRef<EditorInstanceRefs | null>(null)
  if (!refsRef.current)
    refsRef.current = refs ?? createInstanceRefs()
  return (
    <EditorContext value={storeRef.current}>
      <EditorRefsContext value={refsRef.current}>
        <EditorDepsContext value={deps}>{children}</EditorDepsContext>
      </EditorRefsContext>
    </EditorContext>
  )
}
...
export function useEditorDeps(): EditorDeps {
  const deps = use(EditorDepsContext)
  if (!deps)
    throw new Error('useEditorDeps 必须在 <EditorProvider deps=…> 内使用')
  return deps
}
...
export function useEditorRefs(): EditorInstanceRefs {
  const refs = use(EditorRefsContext)
  if (!refs)
    throw new Error('useEditorRefs 必须在 <EditorProvider> 内使用')
  return refs
}
