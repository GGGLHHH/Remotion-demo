...
export const CropOverlay: React.FC<{ scale: number }> = ({ scale }) => {
  ...
  const item = useEditor(s => ((itemId != null && itemId !== '') ? s.undoable.items[itemId] : null)) as
    | CroppableItem
    | null
  ...
  const exit = (): void => editorApi.getState().setItemSelectedForCrop(null)

  const start = (e: React.PointerEvent, mode: ResizeHandle | 'pan'): void => {
    e.stopPropagation()
    if (e.button !== 0)
      return
    dragRef.current = {
  ...
  return (
    ...
    >
      ...
      <div
        className="
          pointer-events-none absolute border border-dashed border-white/40
        "
        style={{
      ...
      >
        {(ghostUrl != null && ghostUrl !== '')
          ? (
              <img
                src={ghostUrl}
                className="object-fill opacity-40 block-full inline-full"
                alt=""
              />
            )
          : (
              <div className="bg-white/10 block-full inline-full" />
            )}
      </div>
      {/* 裁剪窗口 */}
      <div
      ...
      >
        {HANDLES.map(({ handle, x, y, cursor }) => (
          <div
            ...
            className="
              absolute rounded-sm border border-amber-400 bg-white block-2.5
              inline-2.5
            "
