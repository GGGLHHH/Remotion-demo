...
import { getOrderedItems } from '@gedatou/shared'
...
export function hitTest(rect: Rect, rotationDeg: number, px: number, py: number): boolean {
...
function visibleAtFrame(item: EditorStarterItem, frame: number): boolean {
  return frame >= item.from && frame < item.from + item.durationInFrames
}

/** 当前帧可见且命中的最上层 item（渲染顺序逆序找） */
export function topmostItemAt(state: UndoableState, frame: number, px: number, py: number): EditorStarterItem | null {
  const ordered = getOrderedItems(state) // 底层在前
  for (let i = ordered.length - 1; i >= 0; i--) {
    const item = ordered[i]
    if (item.type === 'audio')
      continue // 音频无画面，不参与画布命中
    if (!visibleAtFrame(item, frame))
      continue
    if (hitTest(item, item.rotation, px, py))
      return item
  }
  return null
}
...
export function resizeRect(start: Rect, handle: ResizeHandle, dx: number, dy: number, keepAspect: boolean): Rect {
...
export function toStagePoint(el: Element, clientX: number, clientY: number, scale: number): { x: number, y: number } {
  const rect = el.getBoundingClientRect()
  return { x: (clientX - rect.left) / scale, y: (clientY - rect.top) / scale }
}

/** 拖拽增量(合成坐标):(client - start)/scale */
export function toDelta(startX: number, startY: number, clientX: number, clientY: number, scale: number): { dx: number, dy: number } {
  return {
