...
export const TextEditOverlay: React.FC<{ scale: number }> = ({ scale }) => {
  ...
  const itemId = useEditor(s => s.textItemEditing)
  const item = useEditor(s => ((itemId != null && itemId !== '') ? s.undoable.items[itemId] : null))
  ...
  return (
    <textarea
      ref={ref}
      className="
        absolute z-30 resize-none border border-blue-500 bg-transparent
        outline-none
      "
      style={{
