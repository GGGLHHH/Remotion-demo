...
export const NumberField: React.FC<{
  ...
  // 输入框在编辑期间持有自己的文本(允许中间态如 "-" "1."),外部值变了要覆盖回去。
  ...
  const scrubHandlers = {
    onPointerDown: (e: React.PointerEvent<HTMLElement>) => {
      ...
      scrubRef.current = { x: e.clientX, base: value, moved: false }
    },
  ...
  const input = (extraCls: string): ReactElement => (
    <Input
      type="number"
      className={`
        pe-1 text-end text-xs tabular-nums block-7
        md:text-xs
        ${extraCls}
      `}
      value={text}
  ...
  if (inline) {
    return (
      <label className={`
        relative flex items-center min-inline-0
        ${className ?? ''}
      `}
      >
        <span
          {...scrubHandlers}
          className={`
            absolute inset-s-2 z-10 text-xs text-muted-foreground
            ${handleCls}
          `}
        >
          {label}
  ...
  return (
    <label className={`
      flex flex-col gap-1 min-inline-0
      ${className ?? ''}
    `}
    >
      <span
        {...scrubHandlers}
        className={`
          text-xs text-muted-foreground inline-fit
          ${handleCls}
        `}
      >
        {label}
        ...
        {Icon
          ? (
              <span
                {...scrubHandlers}
                className={`
                  absolute inset-s-2 z-10 text-muted-foreground
                  ${handleCls}
                `}
              >
                <Icon className="block-3.5 inline-3.5" />
              </span>
            )
          : null}
        {input(Icon ? 'pl-7' : '')}
