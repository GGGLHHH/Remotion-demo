// Keyframe 显式点名:DOM 的 Web Animations API 也有个全局 Keyframe,不导入就会解析到它
import type { AnimatableProp, Keyframe, KeyframeEasing } from '@gedatou/shared'
...
export function useItemKeyframes(itemId: string): ItemKeyframesApi {
  ...
  return useMemo<ItemKeyframesApi>(() => {
    const list = (prop: AnimatableProp): Keyframe[] => item?.keyframes?.[prop] ?? []
    return {
