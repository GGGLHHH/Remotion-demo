  ...
  return (
    <Popover
      ...
      onOpenChange={(o) => {
        setOpen(o)
        if (!o)
          setFontHoverPreview(null) // 关闭（点外部/Esc）时清掉悬停预览
      }}
    >
      <PopoverTrigger
        className="
          flex items-center justify-between gap-1.5 rounded-lg border
          border-input bg-transparent px-2 text-start text-xs transition-colors
          outline-none select-none block-7 inline-full min-inline-0
      ...
      <PopoverContent
        side="left"
        align="start"
        className="p-0 inline-(--anchor-width)"
      >
