...
function linearToDb(v: number): number {
  return v <= 0.001 ? -60 : Math.max(-60, Math.min(20, 20 * Math.log10(v)))
}
const dbToLinear = (db: number): number => (db <= -60 ? 0 : 10 ** (db / 20))
...
export const MediaSection: React.FC<{ item: MediaItem }> = ({ item }) => {
  ...
  const setSpeed = (rate: number): void => {
    updateUndoable((s) => {
      ...
      if (capped !== null && next.durationInFrames > capped)
        next.durationInFrames = Math.max(1, capped)
  ...
  const audioRows = (it: VideoItem | AudioItem): ReactElement => {
    ...
    return (
      <>
        <SliderField
          ...
          onChange={v => patch({ volume: dbToLinear(v) }, false)}
        />
