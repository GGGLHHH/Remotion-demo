...
export const LayoutSection: React.FC<{
  ...
  const alignGroup = (defs: typeof ALIGNS): ReactElement => (
  ...
  const setLinkedDim = (prop: 'width' | 'height', v: number, c: boolean): void => {
    ...
    if (!kf.has(prop) && (linked === null || !kf.has(other))) {
      patch((linked === null ? { [prop]: v } : { [prop]: v, [other]: linked }), c)
      return
    }
    animPatch(prop, v, c)
    if (linked !== null)
      animPatch(other, linked, c)
  }
  const setW = (v: number, c: boolean): void => setLinkedDim('width', v, c)
  const setH = (v: number, c: boolean): void => setLinkedDim('height', v, c)
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="flex items-center gap-1">
            <AnimatableNumberField
              ...
              label="X"
              className="flex-1"
              onChange={(v, c) => animPatch('left', v, c)}
            />
          </div>
          <div className="flex items-center gap-1">
            <AnimatableNumberField
              ...
              label="Y"
              className="flex-1"
              onChange={(v, c) => animPatch('top', v, c)}
            />
          </div>
        </div>
      </div>
