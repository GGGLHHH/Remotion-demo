...
export const CaptionsStyleSection: React.FC<{ item: CaptionsItem }> = ({ item }) => {
  ...
  const patchCaption = (index: number, partial: Partial<Caption>): void =>
  ...
  const importSubtitles = async (file: File): Promise<void> => {
  ...
  const exportSubtitles = (): void => {
    ...
    const base = asset?.filename?.replace(/\.(?:captions\.json|srt|vtt)$/i, '')
    a.download = `${base != null && base !== '' ? base : 'captions'}.srt`
    a.click()
  ...
  return (
    <>
      ...
      <Section title={t('captionsPanel.wordCorrection')}>
        <div className="flex flex-col gap-1 overflow-y-auto pe-0.5 max-block-72">
          {/* 逐词字幕没有 id,位置就是它的身份(增删都在末尾,不重排);两个 Input 的 key 还带上内容,
              外部改了这条(导入/whisper 回填)就重挂载,好让非受控 Input 吃到新的 defaultValue */}
          ...
          {captions.map((c, i) => (
            <div key={i} className="flex items-center gap-1">
              ...
              <Input
                ...
                className="
                  shrink-0 px-1.5 text-end text-xs tabular-nums block-7
                  inline-20
                  md:text-xs
                "
              ...
              <Button
                variant="ghost"
                size="icon-sm"
                className="shrink-0 block-7 inline-7"
