...
export const CropSection: React.FC<{
   ...
   */
  const applyCrop = (next: Crop | null, commit: boolean): void => {
  ...
  const setEdge = (edge: keyof typeof edges, pctV: number): void => {
    const total = edge === 'left' || edge === 'right' ? mediaW : mediaH
    const e = { ...edges, [edge]: Math.round((pctV / 100) * total) }
    if (edge === 'left')
      e.left = Math.min(e.left, mediaW - e.right - 1)
    if (edge === 'right')
      e.right = Math.min(e.right, mediaW - e.left - 1)
    if (edge === 'top')
      e.top = Math.min(e.top, mediaH - e.bottom - 1)
    if (edge === 'bottom')
      e.bottom = Math.min(e.bottom, mediaH - e.top - 1)
    applyCrop(
      {
        left: e.left,
        top: e.top,
  ...
  const setPart = (partial: Partial<Crop>, commit: boolean): void => {
    const c = { ...cur, ...partial }
  ...
  const edgeSlider = (label: string, edge: keyof typeof edges): ReactElement => {
    const total = edge === 'left' || edge === 'right' ? mediaW : mediaH
    return (
      <SliderField
        label={label}
