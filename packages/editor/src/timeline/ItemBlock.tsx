...
import type { FadePairKind } from './item-block-math'
import { memo, useMemo, useRef, useState } from 'react'
import { useT } from '#lib/i18n'
import { moveKeyframesAtFrame } from '#lib/keyframe-ops'
import { useEditor, useEditorApi } from '#state/context'
import { Filmstrip } from './Filmstrip'
import { formatDb, gainToTopFraction, readFadePair, topFractionToGain, wedgePath, writeFade } from './item-block-math'
...
export function itemLabel(item: EditorStarterItem): string {
  if (item.type === 'text')
    return item.text.slice(0, 20) || 'Text'
  if (item.type === 'solid')
    return 'Solid'
  if (item.type === 'custom')
...
export const ItemBlock = memo<{
  ...
  const startHandleDrag = (
    ...
    apply(e)
    const onMove = (ev: PointerEvent): void => apply(ev)
    const onUp = (): void => {
  ...
  const onVolumePointerDown = (e: React.PointerEvent): void => {
    // 与修剪手柄一致：按下即独占选中
      ...
      (ev) => {
        ...
        editorApi.getState().updateUndoable(
          (s) => {
            ...
            if (!it || (it.type !== 'video' && it.type !== 'audio') || it.volume === vol)
              return s
  ...
  const onFadePointerDown = (e: React.PointerEvent, side: 'in' | 'out', kind: FadePairKind): void => {
    editorApi.getState().setSelected([item.id])
    ...
    startHandleDrag(
      ...
      (ev) => {
        ...
        if ((side === 'in' ? pair.fadeIn : pair.fadeOut) === v)
          return
   ...
   */
  const onKeyframeDotDown = (e: React.PointerEvent, fromFrame: number): void => {
  ...
  const fadePill = (
    ...
    <div
      {...{ [dataAttr]: side }}
      className={`
        absolute z-40 cursor-ew-resize rounded-b-full bg-white/90 block-[10px]
        inline-[6px]
        ${
    selected || visible ? '' : 'opacity-0'
      ...
      onPointerEnter={() => {
        ...
        if (item.type === 'video')
          setHoverZone(kind === 'visual' ? 'film' : 'strip')
      }}
  ...
  return (
    <div
      ...
      className={`
        group absolute inset-y-px cursor-pointer rounded-sm border text-xs
        text-white/90
        ${COLORS[item.type]}
        ${
    ...
    >
      ...
      <div
        className="
          absolute inset-0 flex items-center overflow-hidden rounded-sm px-2
        "
        style={hasAudioStrip ? { paddingBottom: AUDIO_STRIP_H } : undefined}
      >
        {item.type === 'video' && mediaUrl != null && mediaUrl !== ''
          ? (
        // 胶片区（悬停域 film）：视觉淡变的楔形住在这里（官方顶角一组）
              <div
                className="absolute inset-x-0 inset-bs-0"
              ...
              >
                ...
                {visualFade.fadeIn > 0
                  ? (
                      <svg
                        className="
                          pointer-events-none absolute inset-s-0 inset-bs-0
                        "
                ...
                {visualFade.fadeOut > 0
                  ? (
                      <svg
                        className="
                          pointer-events-none absolute inset-e-0 inset-bs-0
                        "
        ...
        {(hasAudioStrip || isAudio) && (mediaUrl != null && mediaUrl !== '')
          ? (() => {
              ...
              return (
                <div
                  ref={stripRef}
                  className={`
                    absolute inset-x-0 inset-be-0 z-10
                    ${isAudio
                  ? ''
                  : `bg-black/30`}
                  `}
                ...
                >
                  ...
                  >
                    <div
                      className="absolute inset-x-0 bg-white/20 block-px"
        ...
        {!hasAudioStrip && !isAudio && visualFade.fadeOut > 0
          ? (
              <div
                className="pointer-events-none absolute inset-y-0 inset-e-0"
                style={{
        ...
        {item.type === 'video' || isAudio
          ? (
              (filename != null && filename !== '')
                ? (
                    <span className="
                      pointer-events-none absolute inset-s-1 inset-bs-0.5 z-10
                      truncate text-[10px] text-white/90 select-none
                      max-inline-[calc(100%-8px)]
                    "
                    >
                      {filename}
                    </span>
        ...
        {selected && kfFrames.length > 0 && (
          <div
            className="
              pointer-events-none absolute inset-x-0 inset-be-0 z-10 block-2
            "
            data-kf-lane
          >
            {kfFrames.map(f => (
              <button
                ...
                data-kf-dot
                className="
                  pointer-events-auto absolute inset-be-0 -translate-x-1/2
                  rotate-45 border border-background bg-primary block-2 inline-2
                "
                style={{ left: f * zoom }}
      ...
      {dragTip
        ? (
            <div
              className="
                pointer-events-none fixed z-50 flex items-center rounded-sm
                bg-black/90 px-2 text-xs font-medium whitespace-nowrap
                text-neutral-300 tabular-nums block-[26px]
              "
      ...
      <div
        data-trim="start"
        className="
          absolute inset-y-0 -inset-s-px z-30 cursor-e-resize inline-1.5
        "
        onPointerDown={(e) => {
          e.stopPropagation()
      ...
      <div
        data-trim="end"
        className="
          absolute inset-y-0 -inset-e-px z-30 cursor-w-resize inline-1.5
        "
        onPointerDown={(e) => {
          e.stopPropagation()
