...
export function rowHeightOf(st: UndoableState, trackId: string): number {
  let hasAudio = false
  for (const i of dictValues(st.items)) {
    if (i.trackId !== trackId)
      continue
    if (i.type === 'video')
      return MEDIA_TRACK_HEIGHT
    if (i.type === 'audio')
      hasAudio = true
  }
...
export function rowTops(st: UndoableState): number[] {
...
export function trackIndexAtY(st: UndoableState, y: number): number {
  if (y < RULER_HEIGHT)
    return -1
