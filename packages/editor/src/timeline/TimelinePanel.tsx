...
export const TimelinePanel: React.FC<{ className?: string }> = ({ className }) => {
   ...
   */
  const onTransitionPointerDown = (e: React.PointerEvent, tr: Transition, a: EditorStarterItem): void => {
    if (e.button !== 0)
      return
    e.stopPropagation()
  ...
  const dropInfo = (e: React.DragEvent): { frame: number, trackIndex: number } => {
  ...
  const menuReorder = (where: 'front' | 'back'): void => {
    const id = menuItemIdRef.current
    if (!(id != null && id !== ''))
      return
    editorApi
      .getState()
  ...
  const onResizePointerDown = (e: React.PointerEvent): void => {
    ...
    const startH = height
    const onMove = (ev: PointerEvent): void => setHeight(startH + (startY - ev.clientY))
    const onUp = (): void => {
  ...
  if (virtualRowIndex !== null) {
    headerRows.splice(
      virtualRowIndex,
      0,
      <div key="__virtual" className="border-be border-border/50" style={{ height: TRACK_HEIGHT }} />,
    )
  }

  /** 吸附线（移动或修剪） */
  const guideFrame = moveVisual?.guideFrame ?? trimGuide
  const movingItem = (moveVisual ? undoable.items[moveVisual.id] : null) ?? null

  return (
    <div
      ref={panelRef}
      className={cn('relative shrink-0 border-bs border-border bg-card', className)}
      style={{ height }}
    >
      <div
        className="absolute inset-x-0 -inset-bs-1 z-30 cursor-ns-resize block-2"
      ...
      <div className="flex overflow-y-auto" style={{ height: `calc(100% - 2rem)` }}>
        <div className="shrink-0 border-e border-border" style={{ width: HEADER_WIDTH }}>
