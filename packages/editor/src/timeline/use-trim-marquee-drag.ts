...
// 时间线「修剪 / roll 编辑 / 框选」拖拽状态机:三者共享一个 drag ref(判别联合),故整体收进一个 hook。
...
export function useTrimMarqueeDrag(deps: {
  ...
  const startTrim = (e: React.PointerEvent, item: EditorStarterItem, edge: 'start' | 'end') => {
  ...
  const startRoll = (e: React.PointerEvent, aId: string, bId: string) => {
