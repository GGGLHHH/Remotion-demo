 ...
 */
export const TrackHeader = memo<{ track: Track, number: number, height: number }>(({
  track,
  number,
  height,
}) => {
  ...
  const toggle = (key: 'hidden' | 'muted'): void =>
    updateUndoable(s => ({
      ...s,
  ...
  return (
    <div
      className="
        flex items-center gap-1 border-be border-border/50 px-2 text-xs
        text-muted-foreground
      "
      style={{ height }}
    >
