...
export const TimelineTracks: React.FC<{
  ...
  const laneRows = tracks.map((track, ti) => {
    const rowItems = Object.values(items).filter((i) => i.trackId === track.id);
    const rowTransitions = Object.values(transitions).filter((tr) => tr.trackId === track.id);
    return (
      <div key={track.id} className="relative border-b border-border/50" style={{ height: rowHeights[ti] }}>
        {rowItems.map((item) => (
          <ItemBlock
            key={item.id}
            item={item}
