...
export const EditorToolbar: React.FC<{ className?: string, children: React.ReactNode }> = ({
  ...
  <header
    className={cn(`
      flex shrink-0 items-center gap-1.5 border-be border-border px-4 text-sm
      block-12
    `, className)}
  >
    {children}
...
export const EditorTitle: React.FC<{ className?: string, children?: React.ReactNode }> = ({
  className,
  children,
}) => <span className={cn('me-4 font-medium', className)}>{children ?? 'Remotion Editor'}</span>
...
export const ZoomControls: React.FC = () => {
  ...
  const effective = (): number => (canvasZoom === 'fit' ? refs.fitScale.current : canvasZoom)
  return (
    <span className="flex items-center gap-0.5">
      ...
      <span className="
        text-center text-xs text-muted-foreground tabular-nums min-inline-11
      "
      >
