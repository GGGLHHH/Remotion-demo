...
export function useEditorChrome(): void {
  ...
  useEffect(() => {
    if (!hasActiveUploads && !hasActiveRenders && !hasActiveCaptioning)
      return
    // 现代浏览器认 preventDefault() 就够了;e.returnValue 是已废弃的旧写法
...
export const EditorContainer: React.FC<{
  ...
  useEditorChrome()
  return (
    <div className={cn('flex flex-col bg-card text-foreground', fill
      ? `block-full`
      : `block-screen`, className)}
    >
