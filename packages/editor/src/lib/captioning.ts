...
export function audibleSegment(item: { trimBefore: number, playbackRate: number, durationInFrames: number }, fps: number): { offsetSec: number, durationSec: number } {
  return {
...
export function remapCaptionTimes(captions: Caption[], playbackRate: number): Caption[] {
 ...
 */
function attachCaptions(s: UndoableState, src: EditorStarterItem, captions: Caption[], filename: string): UndoableState {
 ...
 */
export async function importCaptionsForItem(store: EditorStoreApi, deps: EditorDeps, itemId: string, file: File): Promise<void> {
  const t = tFor(deps)
 ...
 */
export async function generateCaptions(store: EditorStoreApi, deps: EditorDeps, itemId: string): Promise<void> {
  const t = tFor(deps)
  ...
  const taskId = newId()
  const upsert = (status: 'extracting' | 'transcribing' | 'done' | 'error', error?: string): void =>
