...
import { mergeCaptionPunctuation } from '@gedatou/shared'
...
export function parseSubtitles(input: string): Caption[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (text)
      out.push({ text, startMs, endMs, timestampMs: startMs, confidence: null })
  }
...
function srtTime(ms: number): string {
  const p = (n: number, w = 2): string => String(Math.floor(n)).padStart(w, '0')
 ...
 */
export function serializeSubtitles(captions: Caption[]): string {
  return `${captions
