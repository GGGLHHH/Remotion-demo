 ...
 */
export async function extractWav(url: string, segment?: { offsetSec: number, durationSec: number }): Promise<Blob> {
  const res = await fetch(url)
  if (!res.ok)
    throw new Error(`素材获取失败: ${res.status}`)
