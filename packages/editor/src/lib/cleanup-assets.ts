 ...
 */
export async function cleanupDeletedAssets(store: EditorStoreApi, deps: EditorDeps): Promise<void> {
  store.setState({ past: [], future: [] })
  const { undoable, localUrls } = store.getState()
  const referenced = new Set(
    dictValues(undoable.items).map(i => ('assetId' in i ? i.assetId : null)),
  )
  ...
  for (const { assetId } of undoable.deletedAssets) {
    ...
    if (localUrls[assetId])
      URL.revokeObjectURL(localUrls[assetId])
