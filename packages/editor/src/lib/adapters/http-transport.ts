...
export function createHttpTransport({ baseUrl = '/api' }: { baseUrl?: string } = {}): EditorTransport {
  ...
  return {
    async uploadAsset(file, opts) {
      ...
      if (!res.ok)
        throw new Error(`upload sign failed: ${res.status}`)
    ...
    async generateCaptions(wav) {
      ...
      if (!res.ok)
        throw new Error(`转录失败: ${res.status} ${(await res.text()).slice(0, 200)}`)
      // fetch 的 json() 只能给出 any,断言成契约类型(runtime.ts 的 EditorTransport)
      return res.json() as Promise<{ captions: Caption[] }>
    },
  }
}
