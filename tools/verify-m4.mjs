...
// --- 1) 文本面板：选中文本项 → 改字号/对齐/背景 ---
{
  ...
  let s = await S()
  if (s.items[id].fontSize !== 120)
    fail(`fontSize ${s.items[id].fontSize}`)
  await page.locator('label:has-text("对齐")').nth(1).locator('button').first().click()
  s = await S()
  if (s.items[id].textAlign !== 'left')
    fail('textAlign not left')
  ...
  if ((await bgCheck.getAttribute('aria-checked')) !== 'true')
    await bgCheck.click()
  s = await S()
  if (s.items[id].backgroundColor === null)
    fail('backgroundColor still null')
}
...
// --- 2) 字体选择器：打开 → 选一个字体 ---
{
  ...
  const s = await S()
  if (s.items[id].fontFamily !== 'Roboto')
    fail(`fontFamily ${s.items[id].fontFamily}`)
}
...
// --- 3) 行内编辑：程序化进入 → 输入 → blur 提交 ---
{
  ...
  await ta.blur()
  const s = await S()
  if (s.items[id].text !== '新标题')
    fail(`inline edit text: ${s.items[id].text}`)
  if (s.editing !== null)
    fail('editing not cleared')
}
...
// --- 4) 变速：solid 无媒体面板；用 media? demo 无视频 → 跳过变速，改测淡入 ---
{
  ...
  const s = await S()
  if (s.items[id].fadeInDurationInFrames !== 15)
    fail(`fadeIn ${s.items[id].fadeInDurationInFrames}`)
}
...
// --- 5) 画布框选：先缩小 solid 留出空白，从空白处框选命中它 ---
{
  ...
  if (s.selected.length !== 1)
    fail(`canvas marquee selected ${s.selected.length}`)
...
// --- 6) 右键菜单：solid 置于顶层 → 轨道顺序变化 ---
{
  ...
  const s = await S()
  if (s.items[id].trackId === trackBefore)
    fail('bring-to-front no-op')
  if (s.tracks[0].id !== s.items[id].trackId)
    fail('solid not on top track')
...
// --- 7) 剪贴板：复制粘贴 → 新 item 落在播放头、位置不偏移（官方语义） ---
{
  ...
  const s = await S()
  if (Object.keys(s.items).length !== count0 + 1)
    fail('paste did not add item')
  const newId = s.selected[0]
  const ph = await page.evaluate(() => window.__playerRef.current.getCurrentFrame())
  if (s.items[newId].left !== s.items[id].left)
    fail('paste should keep position')
  if (s.items[newId].from !== ph)
    fail(`paste at frame ${s.items[newId].from}, want playhead ${ph}`)
  await page.keyboard.press('Meta+z')
}
...
// --- 8) Cmd+D 复制体 ---
{
  ...
  const s = await S()
  if (Object.keys(s.items).length !== count0 + 1)
    fail('duplicate did not add item')
  await page.keyboard.press('Meta+z')
}
