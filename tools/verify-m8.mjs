...
if (!srcInfo)
  fail('源信息 section missing')
if (!(await page.getByText('audio.wav').count()))
  fail('源信息 filename missing')
...
if (!(newVol < 1))
  fail(`volume not lowered: ${newVol}`)
...
await page.mouse.up()
s = await getState()
if (!(s.undoable.items[audio.id].fadeInDurationInFrames > 0))
  fail('fadeIn not set by handle drag')
...
await page.mouse.up()
s = await getState()
if (!(s.undoable.items[audio.id].durationInFrames < durBefore))
  fail('trim-end shrink failed')
...
await page.mouse.up()
if (!hatch)
  fail('max-trim indicator not shown during trim drag')
...
if (Object.keys(s.undoable.items).length !== before + 1 || !solids.length)
  fail('solid not drawn')
if (!(solids[0].width > 10 && solids[0].height > 10))
  fail(`solid rect too small: ${solids[0].width}x${solids[0].height}`)
if ((await page.locator('button[title*="绘制色块"]').getAttribute('aria-pressed')) === 'true')
...
if (!dropped)
  fail('dropped image item not created')
if (!(dropped.from > 0))
  fail(`dropped item frame ${dropped.from}, want > 0`)
...
if (!(afterSplit > beforeSplit))
  fail(`scissors split created no items (${beforeSplit} -> ${afterSplit})`)
