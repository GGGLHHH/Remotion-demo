...
// --- 1) 改属性 → Cmd+S → reload → 改动还原、脏标记消失 ---
{
  ...
  s = await S()
  if (!s.dirty)
    fail('not dirty after edit')
  await page.keyboard.press('Meta+s')
  s = await S()
  if (s.dirty)
    fail('still dirty after Cmd+S')
  await page.reload({ waitUntil: 'networkidle' })
  s = await S()
  if (s.items[id]?.fontSize !== 99)
    fail(`fontSize after reload: ${s.items[id]?.fontSize}`)
  if (s.dirty)
    fail('dirty after reload')
}
...
// --- 2) #state= base64 URL 加载 ---
{
  ...
  const s = await S()
  if (s.items[id]?.text !== 'HASH-OK')
    fail(`hash state text: ${s.items[id]?.text}`)
...
// --- 3) 时间码：0:00.00 格式 + 跳尾变化 ---
{
  ...
  if (!/^0:00\.00 \/ \d+:\d{2}\.\d{2}$/.test(before))
    fail(`timecode format: ${before}`)
  ...
  if (!/^\d+:\d{2}\.\d{2} \//.test(after))
    fail(`timecode after seek: ${after}`)
}
...
// --- 4) 循环按钮切换 store.loop ---
{
  ...
  let s = await S()
  if (s.loop !== !s0.loop)
    fail('loop did not toggle')
  await page.getByTitle('循环').click()
  s = await S()
  if (s.loop !== s0.loop)
    fail('loop did not toggle back')
}
...
// --- 5) 素材清理端到端：PUT 到 MinIO → 引用 → 删除 → 清理 → 远端 404 ---
{
  ...
  if (putRes.status !== 200)
    fail(`PUT to MinIO: ${putRes.status}`)
  if ((await fetch(up.publicUrl)).status !== 200)
    fail('uploaded object not readable')
  ...
  const s = await S()
  if (!s.deletedAssets.some(d => d.assetId === 'e2e-asset'))
    fail('asset not in deletedAssets')
  ...
  if (status !== 404)
    fail(`remote object status after cleanup: ${status}`)
}
...
// ---- 播放跟随不抢用户滚动：右滚后不被拽回；循环回跳时视口跟回 ----
{
  ...
  if (sl < 250)
    fail(`follow-scroll yanked user scroll back: ${sl}`)
