...
const audioId = await page.evaluate(
  () =>
    Object.values(window.__editorStore.getState().undoable.items).find(i => i.type === 'audio').id,
)
...
if (task.status !== 'done')
  fail(`captioning task error: ${task.error}`)
...
if (!cap)
  fail('no captions item created')
if (cap.asset?.type !== 'caption' || cap.asset.captions.length === 0)
  fail('caption asset empty')
const fullText = cap.asset.captions.map(c => c.text).join('')
if (!/hello/i.test(fullText))
  fail(`transcript missing "hello": ${fullText}`)
if (cap.item.from !== 0 || cap.item.durationInFrames < 1)
  fail('captions item timing wrong')
...
await page.waitForFunction(
  (w) => {
    ...
    if (!el || !el.textContent.toLowerCase().includes(w))
      return false
    // 当前词应套用高亮色 #facc15
