...
if (s.w !== 1080 || s.h !== 1920)
  fail('unexpected comp size')
await page.getByRole('button', { name: /交换尺寸/ }).click()
s = await getStore()
if (s.w !== 1920 || s.h !== 1080)
  fail(`swap failed: ${s.w}x${s.h}`)
await page.keyboard.press(process.platform === 'darwin' ? 'Meta+z' : 'Control+z')
s = await getStore()
if (s.w !== 1080)
  fail('swap undo failed')
...
s = await getStore()
if (s.items[bgId].left !== 200)
  fail(`X commit failed: ${s.items[bgId].left}`)
await page.keyboard.press(process.platform === 'darwin' ? 'Meta+z' : 'Control+z')
s = await getStore()
if (s.items[bgId].left !== 0)
  fail('X undo failed')
