...
if (s.past !== 1)
  fail(`drag should be 1 undo entry, past=${s.past}`)

// 3) Cmd+Z 撤销回原位
await page.keyboard.press(process.platform === 'darwin' ? 'Meta+z' : 'Control+z')
s = await getStore()
if (s.items[titleId].left !== leftBefore)
  fail('undo did not restore position')
...
s = await getStore()
if (s.zoom === 'fit')
  fail('zoom + did not change')
await page.keyboard.press('0')
s = await getStore()
if (s.zoom !== 'fit')
  fail('zoom 0 did not reset')
