...
if (!video || !audio || !image)
  fail('missing imported items')

// 视频元数据
const vAsset = s.assets[video.assetId]
if (vAsset.width !== 640 || vAsset.height !== 360)
  fail(`video dims ${vAsset.width}x${vAsset.height}`)
if (Math.abs(vAsset.durationInSeconds - 2) > 0.2)
  fail(`video duration ${vAsset.durationInSeconds}`)
if (!vAsset.hasAudio)
  fail('video hasAudio should be true')
if (video.durationInFrames < 55 || video.durationInFrames > 65)
  fail(`video item frames ${video.durationInFrames}`)
// 视觉适配：640x360 缩放到宽 1080 内
if (video.width !== 640)
  fail(`video item width ${video.width} (no upscale expected)`)

// 远端 URL 可匿名读取
for (const a of [vAsset, s.assets[audio.assetId], s.assets[image.assetId]]) {
  if (!a.url.startsWith('http://localhost:9100/'))
    fail(`asset url not remote: ${a.url}`)
  const res = await fetch(a.url)
  if (!res.ok)
    fail(`remote GET ${a.url} -> ${res.status}`)
}
...
if (s.tracks.length !== 4)
  fail(`tracks ${s.tracks.length}, want 4`)
if (video.trackId !== track1Id)
  fail('video should land on existing Track 1')
if (image.trackId !== s.tracks[0].id)
  fail('image should be on new top track')
if (audio.trackId !== s.tracks[1].id)
  fail('audio should be on second new track')
