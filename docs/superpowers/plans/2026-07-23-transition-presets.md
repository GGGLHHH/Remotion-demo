...
export interface TransitionPreset { id: string, type: TransitionType, direction?: TransitionDirection, label: string }
...
export function presetIdOf(t: Pick<Transition, 'type' | 'direction'>): string {
  return TRANSITION_PRESETS.find(p => p.type === t.type && p.direction === t.direction)?.id ?? 'fade'
}
```
...
```ts
import type { EditorStarterItem, Transition, TransitionDirection, TransitionType, UndoableState } from '../types'
...
export function transitionVisual(type: TransitionType, direction: TransitionDirection | undefined, role: 'in' | 'out', p: number): TransitionRenderProps {
  if (type === 'slide') {
    ...
    const [ex, ey] = exit
    if (role === 'out')
      return { opacity: 1, translate: `${ex * 100 * p}% ${ey * 100 * p}%` }
    return { opacity: 1, translate: `${-ex * 100 * (1 - p)}% ${-ey * 100 * (1 - p)}%` }
  }
  if (type === 'wipe') {
    if (role === 'out')
      return { opacity: 1 } // 出场不裁,入场盖住
...
<Row label="Type">
  <Popover open={presetOpen} onOpenChange={setPresetOpen}>
    <PopoverTrigger render={<Button variant="outline" size="sm" className="
      gap-1
    " />}>
      {TRANSITION_PRESETS.find(p => p.id === presetIdOf(t))?.label ?? 'Cross Dissolve'}
      <ChevronDownIcon />
    </PopoverTrigger>
    <PopoverContent align="start" className="inline-40 gap-0.5 p-1">
